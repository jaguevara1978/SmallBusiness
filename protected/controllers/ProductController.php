...
class ProductController extends Controller {
	...
	public function actionCreate($asDialog=false) {
		$model=new Product;
		$filterModel=new Product('search');
			...
			if($model->save()) {
				if (Yii::app()->request->isAjaxRequest && $asDialog) {
					echo CJSON::encode(array(
							...
							'value'=>array(0=>$model->id, 1=>'('.$model->measureUnit->code.') '.$model->code, 2=>$model->name),
					));
					exit;
				} else {
					$model=new Product;
	...
	public function actionUpdateMeasureUnit() {
		$es = new EditableSaver('Product');
		$es->update();
	}

	public function actionUpdateCustomOrder() {
		$es = new EditableSaver('Product');
		$es->update();
	}

	public function actionUpdateDefaultQty() {
		$es = new EditableSaver('Product');
		$es->update();
	}
	...
	public function actionUpdateStockMovement() {
		$es = new EditableSaver('Product');
		$es->update();
	}
}