...
*/
class ProviderInvoiceDetailController extends Controller {
	/**
	...
	public function actionCreate($headerId=null) {
		$model=new ProviderInvoiceDetail;
		$filterModel=new ProviderInvoiceDetail('search');
		if (intval(Yii::app()->request->getParam('clearFilters'))==1)
			ButtonColumnClearFilters::clearFilters($this,$filterModel);
		
		if(isset($_POST['ProviderInvoiceDetail'])) {
			$model->attributes=$_POST['ProviderInvoiceDetail'];
			if($model->save()) {
				$model=new ProviderInvoiceDetail;
	...
	public function actionIndex() {
		$dataProvider=new CActiveDataProvider('ProviderInvoiceDetail');
	...
	public function actionAdmin() {
		$model=new ProviderInvoiceDetail('search');
		if (intval(Yii::app()->request->getParam('clearFilters'))==1)
	...
	public function loadModel($id) {
		$model=ProviderInvoiceDetail::model()->findByPk($id);
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='provider-invoice-detail-form') {
	...
	public function actionUpdateId() {
		$es = new EditableSaver('ProviderInvoiceDetail');
		$es->update();
	}
