...
class StockController extends Controller {
			...
			if($model->save()) {
				if (Yii::app()->request->isAjaxRequest && $asDialog) {
					echo CJSON::encode(array(
							...
							'value'=>array(0=>$model->id, 1=>$model->name,),
					));
					exit;
				} else {
					$model=new Stock;
	...
	public function actionProductStock() {
		...
		if (intval(Yii::app()->request->getParam('clearFilters'))==1)
			ButtonColumnClearFilters::clearFilters($this,$model);
		$this->render('productStock',array('model'=>$model,));
	}
	...
	public function actionUpdateInitStockQty() {
		$es = new EditableSaver('Stock');
		$es->update();
	}

	public function actionUpdateFinalStockQty() {
		$es = new EditableSaver('Stock');
		$es->update();
	}

	public function actionUpdateMovementType() {
		$es = new EditableSaver('Stock');
		$es->update();
	}

	public function actionUpdateClientInvoiceDetail() {
		$es = new EditableSaver('Stock');
		$es->update();
	}

	public function actionUpdateProviderInvoiceDetail() {
		$es = new EditableSaver('Stock');
		$es->update();
	}

	public function actionUpdateManualUnitValue() {
		$es = new EditableSaver('Stock');
		$es->update();
	}
}
