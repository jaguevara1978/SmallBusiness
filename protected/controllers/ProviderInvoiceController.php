...
class ProviderInvoiceController extends Controller {
	...
	public function actionCreate($asDialog=false,$headerId=null) {
		$model=new ProviderInvoice;
		$filterModel=new ProviderInvoice('search');
		if (intval(Yii::app()->request->getParam('clearFilters'))==1)
			ButtonColumnClearFilters::clearFilters($this,$filterModel);
		
		if(isset($_POST['ProviderInvoice'])) {
			$model->attributes=$_POST['ProviderInvoice'];
			if($model->save()) {
				Yii::app()->user->setFlash('success',Yii::t('app','message.successFullySaved'));
				$this->redirect(array('/ProviderInvoiceDetail/create','headerId'=>$model->id,));
			}
		}
	...
	public function actionIndex() {
		$dataProvider=new CActiveDataProvider('ProviderInvoice');
	...
	public function actionAdmin() {
		$model=new ProviderInvoice('search');
		if (intval(Yii::app()->request->getParam('clearFilters'))==1)
	...
	protected function performAjaxValidation($model) {
		if(isset($_POST['ajax']) && $_POST['ajax']==='provider-invoice-form') {
	...
	public function actionUpdateId() {
		$es = new EditableSaver('ProviderInvoice');
		$es->update();
	}
	...
	public function actionUpdatePaymentDate() {
		$es = new EditableSaver('ProviderInvoice');
		$es->update();
		ProviderInvoiceDeposit::model()->updateAll(array('date'=>$es->value),'client_invoice=:id AND final_payment=1',array(':id'=>$es->primaryKey));
	}
	...
	public function actionPayInvoice($id) {
		ProviderInvoice::pay($id);
		$this->redirect(array('create'));
	}

	public function actionLoadStock($id) {
		$model=$this->loadModel($id);
		$model->status=Yii::app()->params['ProviderInvoice_stock_status'];
		$model->update();
		$this->redirect(array('create'));
	}
}